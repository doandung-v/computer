<?php 
	trait HomeModel{
		...
		public function modelGetProducts($category_id){
			...
			$query = $conn->query("select * from products where category_id in (select id from categories where id=$category_id or parent_id=$category_id) order by id asc");
			$result = $query->fetchAll();
			return $result;
		}
		public function modelGetHotNews(){
			$conn = Connection::getInstance();
			$query = $conn->query("select * from news where hot=1 order by id asc");
			$result = $query->fetchAll();
			return $result;
		}
	}
